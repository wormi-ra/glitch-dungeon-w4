...
OBJECTS = $(patsubst src/%.c, build/%.o, $(wildcard src/*.c src/**/*.c))
OBJECTS += $(patsubst src/%.cpp, build/%.o, $(wildcard src/*.cpp src/**/*.cpp))
DEPS = $(OBJECTS:.o=.d)
...
ifeq ($(DETECTED_OS), Windows)
	...
	MKDIR_PATH = if not exist "$(@D)" md "$(@D)"
	RMDIR = rd /s /q
else
	MKDIR_BUILD = mkdir -p build
	MKDIR_PATH = mkdir -p "$(@D)"
	RMDIR = rm -rf
endif
