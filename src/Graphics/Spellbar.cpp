...
void Spellbar::draw(const Viewport &view) const {
    ...
    for (const auto &spell : Game::player.spellbook) {
        ...
        IntRect rect = view.transform({this->position + position + Vector2{1, 3}, {13, 13}});
        ...
        *DRAW_COLORS = 0x1230;
        Data::SPELL_ICONS.blitSub(view, uint8_t(spell->getType()) - 1, this->position + position + Vector2{2, 4});
        position.x += 16;
    }
    IntRect textRect = view.transform({this->position + Vector2{2, 24}, this->size}); 
    if (text != nullptr) {
